...
bool 
checkFilename(GTKString filename, int mode) {
	//QFile* file = new QFile(filename);
	//return file->open(mode);
}

void
gtk_gui::fileError(QString smilfilename) {
 	char buf[1024];
...
bool 
gtk_gui::openSMILfile(const GTKString smilfilename, int mode) {
...
void
gtk_gui::setDocument(const GTKString& smilfilename) {
#ifdef	QT_NO_FILEDIALOG	/* Assume embedded Qt */
	if (openSMILfile(smilfilename, IO_ReadOnly))
		slot_play();
#endif/*QT_NO_FILEDIALOG*/
...
#ifdef	QT_NO_FILEDIALOG	/* Assume embedded Qt */
	GKTString* smilfilepointer = new QString(selected_file.file());
	GTKString smilfilename = *smilfilepointer;
