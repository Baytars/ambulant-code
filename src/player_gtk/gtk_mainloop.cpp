...
gtk_mainloop::create_document(const char *filename)
{
	...
	if (url.is_local_file() && !url.is_absolute()) {
		...
		//AM_DBG 
		m_logger->debug("mainloop::create_document: URL is now \"%s\"", url.get_url().c_str());
	}
	size_t size;
...
void
gtk_mainloop::open(const net::url newdoc, bool start, common::player *old)
{
	const char* document_name(newdoc.get_url().c_str());
	AM_DBG m_logger->trace("gtk_mainloop::open \"%s\"",document_name);
