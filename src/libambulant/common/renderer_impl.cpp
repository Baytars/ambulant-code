...
renderer_playable_ds::~renderer_playable_ds()
{
    /*KB AM_DBG*/ lib::logger::get_logger()->debug("~renderer_playable_ds(0x%x)", (void *)this);
  	if (m_src) {
		m_src->release();
		m_src = NULL;
	}
...
{
    /*KB AM_DBG*/ lib::logger::get_logger()->debug("renderer_playable_ds.start(0x%x %s)", (void *)this, m_node->get_sig().c_str());
	if (m_activated) {
