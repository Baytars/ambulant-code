		...
		if (!m_buffer.buffer_full()) {
			...
			if (outbuf) {
				if(inbuf) {
					...
					AVPacket gb_avp;
					av_init_packet(&gb_avp);
					gb_avp.data = inbuf;
					gb_avp.size = cursz;
					//int decoded = avcodec_decode_audio2(m_con, ffmpeg_outbuf, &outsize, inbuf, cursz);
					int decoded = avcodec_decode_audio3(m_con, ffmpeg_outbuf, &outsize, &gb_avp);
					...
					while (decoded > 0 && decoded < cursz) {
						...
						decoded = avcodec_decode_audio3(m_con, (short*) ffmpeg_outbuf, &outsize, &gb_avp);
						if (decoded < 0) outsize = 0;
