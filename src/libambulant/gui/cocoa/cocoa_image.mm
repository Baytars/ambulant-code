...
cocoa_image_renderer::redraw_body(const rect &dirty, gui_window *window)
{
	...
	if (m_node->get_attribute("backgroundImage") && m_dest->is_tiled()) {
		...
		for(it=tiles.begin(); it!=tiles.end(); it++) {
			...
			[m_image drawInRect: cocoa_dstrect fromRect: cocoa_srcrect operation: NSCompositeSourceOver fraction: (CGFloat)1.0];
		}
		m_lock.leave();
		return;
	}
	...
	[m_image drawInRect: cocoa_dstrect fromRect: cocoa_srcrect operation: NSCompositeSourceOver fraction: (CGFloat)alfa];
#else
	[m_image drawInRect: cocoa_dstrect fromRect: cocoa_srcrect operation: NSCompositeSourceOver fraction: (CGFloat)1.0];
#endif
